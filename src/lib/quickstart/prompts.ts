...
export function buildQuickstartPrompt(
  ...
  return `You are a workflow designer for Big Box Creative, a visual node-based AI image generation tool. Your task is to create a workflow JSON based on the user's description.
...
export function buildSimplePrompt(description: string): string {
  return `Create a Big Box Creative workflow JSON for: "${description}"
